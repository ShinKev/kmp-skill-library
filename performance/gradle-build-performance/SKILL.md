...
### 13. Avoid Default KMP Hierarchy Template (KMP projects only)

The default hierarchy template (auto-enabled since Kotlin 1.9.20) creates intermediate source sets for every possible target combination (`nativeMain`, `appleMain`, `tvosMain`, `watchosMain`, etc.) even when you don't use them. This adds ~8 empty tasks per module. In a 70-module project this inflated sync time from 15 min → 80 min; in a 180-module project it caused complete unusability.

**Symptom**: Long configuration or IDE sync times in KMP projects; many tasks with `native`/`apple` in the name completing instantly on modules that only target Android + iOS.

**Fix**: Replace the implicit default with an explicit hierarchy matching only your actual targets:
...
kotlin {
    applyHierarchyTemplate {
        common {
            ...
            // Only add withJvm(), withWasmJs(), etc. if you actually declare those targets
        }
    }
}
```

Move this into your `KmpLibraryConventionPlugin` so every KMP module gets it automatically.
