...
description: Audit and improve Compose Multiplatform (CMP) runtime performance from code review and architecture. Use when asked to diagnose slow rendering, janky scrolling, excessive recompositions, or performance issues in shared or Android Compose UI.
---
...
Audit Compose Multiplatform (CMP) view performance end-to-end, from instrumentation and baselining to root-cause analysis and concrete remediation steps. Most patterns apply equally to Android and iOS targets; profiling tools differ per platform.
...
**iOS:**
- Use **Instruments** (Xcode) — the Time Profiler and Core Animation instruments for frame timing.
- The code-first audit workflow (Section 1) applies equally to CMP on iOS.

Ask for:
- Layout Inspector screenshot showing recomposition counts (Android).
- Perfetto or Instruments trace export.
...
- **Async image loading**: Use Coil 3 (KMP-compatible) with proper sizing constraints. See the `coil-compose` skill.
