---
name: compose-ui
description: Best practices for building UI with Compose Multiplatform (CMP), focusing on state hoisting, performance optimizations, and theming. Shared composables live in commonMain. Use this when writing or refactoring Composable functions in KMP/CMP projects.
---
...
*   **ViewModel Integration**: The screen-level Composable retrieves state from the ViewModel. In `commonMain`, use `collectAsState()`; in Android-specific composables, `collectAsStateWithLifecycle()` is preferred. Pass state down as plain values.
