...
description: Authoritative rules and patterns for production-quality Kotlin Coroutines on Android. Covers structured concurrency, lifecycle integration, and reactive streams. Android-specific — for KMP shared coroutines patterns see kotlin-concurrency-expert.
---

> **Platform Scope: Android-Specific**
> This skill covers Android lifecycle integration (`viewModelScope` collection, `lifecycleScope`, `repeatOnLifecycle`, `collectAsStateWithLifecycle`). These APIs must only appear in `androidMain` or `androidApp`. For coroutine patterns that apply to all KMP targets (shared repositories, Ktor, SQLDelight), see the `kotlin-concurrency-expert` skill.
