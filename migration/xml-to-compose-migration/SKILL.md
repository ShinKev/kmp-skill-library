...
> This skill applies only to Android (`:androidApp` or `androidMain`). XML layouts are an Android-only concept. KMP/CMP new UI should use Compose Multiplatform in `commonMain` from the start.
